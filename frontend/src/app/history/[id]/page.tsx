  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-[10px] text-white/40 font-black uppercase tracking-widest">Memuat Detail...</p>
      </div>
    );
  }
  ...
  if (error || !visit) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center space-y-4">
        ...
        <h2 className="text-xl font-black text-white uppercase italic">Opps! Terjadi Kesalahan</h2>
        <p className="text-sm text-white/40 font-medium">{error || "Data tidak ditemukan"}</p>
        <button 
          onClick={() => router.back()}
          className="px-6 py-3 bg-secondary text-white rounded-2xl font-black uppercase text-[10px]"
        >
          Kembali ke Riwayat
        </button>
      </div>
  ...
  return (
    <div className="space-y-6 pb-20 max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={() => router.back()}
          className="p-2.5 rounded-2xl bg-white/5 border border-white/10 text-white/70 active:scale-95 transition-all text-xs flex items-center gap-2"
        >
          <ChevronLeft className="w-4 h-4" /> Kembali
        </button>
        <span className="text-[10px] uppercase font-black text-secondary tracking-widest px-4 py-1.5 rounded-full bg-secondary/10 border border-secondary/10">
          Detail Laporan
        </span>
      </div>
      ...
      <section className="space-y-4">
        <h3 className="text-xs font-black text-white/40 uppercase tracking-widest px-1 flex items-center gap-2">
        ...
        <div className="space-y-3">
          {visit.responses && visit.responses.length > 0 ? (
            visit.responses.map((resp) => {
              const isFile = resp.value?.includes('uploads/');
              return (
                <div key={resp.id} className="bg-white/5 border border-white/5 p-4 rounded-3xl flex items-center justify-between gap-4">
                  <div className="space-y-1">
                    <p className="text-[10px] font-black text-white/70 uppercase leading-snug">
                      {resp.template?.label || `Parameter #${resp.template_id}`}
                    </p>
                  </div>
                  <div className="flex-shrink-0">
                    {isFile ? (
                      <div className="w-14 h-14 rounded-xl bg-white/5 border border-white/10 overflow-hidden relative group">
                        <img 
                          src={`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}/storage/${resp.value}`} 
                          className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"
                          alt="Param"
                          onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}/storage/${resp.value}`, '_blank')}
                        />
                      ...
                      <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase ${resp.value === 'Ya' || resp.value === 'Ok' ? 'bg-success/20 text-success border border-success/10' : 'bg-secondary/20 text-secondary border border-secondary/10'}`}>
                        {resp.value}
                      </span>
                    )}
                  </div>
            ...
            <div className="p-10 text-center bg-white/5 rounded-3xl border border-dashed border-white/10">
              <p className="text-[10px] text-white/30 font-bold uppercase">Tidak ada data checklist</p>
            </div>
          )}
        </div>
      </section>
        ...
        {visit.findings && visit.findings.length > 0 ? (
          <div className="space-y-4">
            {visit.findings.map((finding) => (
              <div key={finding.id} className="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden">
                <div className="p-5 flex gap-4">
                  {finding.foto_temuan && (
                    <div className="w-20 h-20 rounded-2xl bg-white/5 border border-white/10 overflow-hidden flex-shrink-0">
                      <img 
                        src={`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}/storage/${finding.foto_temuan}`} 
                        className="w-full h-full object-cover"
                        alt="Finding"
                        onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}/storage/${finding.foto_temuan}`, '_blank')}
                      />
                    </div>
                  )}
                  <div className="space-y-2 flex-1">
                    <div className="flex justify-between items-start">
                      <span className="text-[8px] font-black text-white/30 uppercase tracking-widest">TIKET: {finding.nomor_tiket}</span>
                    ...
                    <p className="text-xs text-white font-medium leading-relaxed italic line-clamp-2">
                       &quot;{finding.temuan}&quot;
                    </p>
                  </div>
                </div>
          ...
          <div className="p-8 text-center bg-white/5 rounded-3xl border border-dashed border-white/10">
            <p className="text-[10px] text-white/30 font-bold uppercase">Semua Kondisi Normal / Tidak Ada Temuan</p>
          </div>
        )}
      </section>
    </div>
